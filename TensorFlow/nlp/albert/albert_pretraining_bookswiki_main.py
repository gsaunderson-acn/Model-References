...
class AlbertPretrainingBookswiki(TrainingRunHWConfig):
    def __init__(self, scaleout, num_workers_per_hls, hls_type, kubernetes_run, args, p1_steps, p1_warmup, p1_batch_size, p1_max_seq_len,
                p2_steps, p2_warmup, p2_batch_size, p2_max_seq_len, pretrained_model, enable_scoped_allocator, save_ckpt_steps):
    ...
    def build_command_phase1(self):
        try:
            ...
            dataset_path_phase1 = Path(self.args.dataset_path).joinpath(f"seq_len_{seq_len}")
            ...
            self.command = (
                ...
                f" --output_dir={results_dir_phase1}"
                f" --do_train=True"
                f" --do_eval=True"
    ...
    def build_command_phase2(self):
        try:
            ...
            dataset_path_phase2 = Path(self.args.dataset_path).joinpath(f"seq_len_{seq_len}")
            ...
            self.command = (
                ...
                f" --input_file={training_file_path}/*"
                f" --eval_file={eval_file_path}/*"
                f" --output_dir={results_dir_phase2}"
                f" --do_train=True"
                f" --do_eval=True"
                f" --albert_config_file={albert_config}"
                f" --init_checkpoint={init_checkpoint}"
