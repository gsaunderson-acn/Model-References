...
## Table Of Contents
  ...
  * [Training](#training)
    ...
    * [The `demo_resnext.py` script](#the-demo_resnextpy-script)
  * [Examples](#examples)
  * [Changelog](#changelog)
...
optional arguments:
        ...
        -ebe <epochs>      --epochs_between_evals <epochs>      Number of training epochs between evaluations, defaults to 1.
                           ...
                           --profiling`                         Turn on profiling (generate json every for steps 20-24) (default: False)

examples:

        $PYTHON demo_resnext.py -bs 128 -dt bf16 -te 90 -ebe 90 --data_dir /data/tensorflow/imagenet/tf_records/
        $PYTHON demo_resnext.py -bs 128 -dt bf16 -dlit bf16 -te 90 -ebe 90 --num_workers_per_hls 8 --data_dir /data/tensorflow/imagenet/tf_records/
...
- ResNeXt101, bf16, batch size 128, 90 epochs, experimental preloading
    ```bash
    $PYTHON demo_resnext.py -dt bf16 -dlit bf16 -bs 128 -te 90 -ebe 90 --data_dir /data/tensorflow/imagenet/tf_records/
    ```
...
- ResNeXt101, bf16, batch size 128, 90 epochs, experimental preloading
    ```bash
    $PYTHON demo_resnext.py -dt bf16 -dlit bf16 -bs 128 -te 90 -ebe 90 --num_workers_per_hls 8 --data_dir /data/tensorflow/imagenet/tf_records/
    ```
...
- ResNeXt101, bf16, batch size 128, 90 epochs, no experimental preloading
    ```bash
    mpirun --allow-run-as-root --tag-output --merge-stderr-to-stdout --output-filename /root/tmp/demo_resnext_log --bind-to core --map-by socket:PE=7 -np 8 $PYTHON Model-References/TensorFlow/computer_vision/Resnets/ResNeXt/imagenet_main.py --use_horovod -dt bf16 -dlit bf16 -bs 128 -te 90 -ebe 90 --data_dir /data/tensorflow/imagenet/tf_records/
    ```
...
```bash
export MULTI_HLS_IPS=IP1,IP2,IP3...
export MPI_TCP_INCLUDE=interface_name
$PYTHON demo_resnext.py -dt bf16 -dlit bf16 -bs 128 -te 90 -ebe 90 --weight_decay 7.93E-05 --num_workers_per_hls 8
```
